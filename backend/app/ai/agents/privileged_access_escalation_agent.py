"""
backend/app/ai/agents/privileged_access_escalation_agent.py
──────────────────────────────
Tier-2 Escalation Agent: handles account lockouts, managerial escalations and
other sensitive matters.

Tooling:
  • MCP stdio server (backend/app/mcp/server.py) → read-only AD lookups
...
from backend.app.ai.prompts.escalation_prompt import ESCALATION_SYSTEM_PROMPT
...
def create_privileged_access_escalation_agent(client: OpenAIChatCompletionClient, history_provider: "HistoryProvider") -> af.Agent:
